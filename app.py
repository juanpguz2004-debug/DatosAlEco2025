...
@st.cache_data
def load_data():
    # Nombre de archivo REVERTIDO a la versión con guiones bajos
    ...
    try:
        ...
        if 'ANO_DE_CORTE' in df.columns:
            ...
            # FIX ROBUSTO: Redondea y convierte a entero, rellenando con -1.
            df['ANO_DE_CORTE'] = df['ANO_DE_CORTE'].round(0).fillna(-1).astype(int)
            
        # Filtra solo los datos válidos después de la limpieza
...
def load_assets():
    ...
    if not files_exist:
        st.error("❌ Error: Faltan archivos .pkl del modelo. Asegúrate de que los CINCO archivos .pkl estén en la misma carpeta.")
...
if df.empty:
    st.error("❌ ERROR FATAL: No se encontraron datos válidos en el CSV. Confirma la ubicación y el contenido del archivo.")
    st.stop()
    
if None in [model_cls, model_reg_gan, model_reg_per, MODEL_FEATURE_NAMES, label_encoders]:
    st.error("❌ ERROR FATAL: No se pudieron cargar los CINCO modelos o las referencias. Verifica los archivos .pkl.")
    st.stop()
...
try:
    ...
    row_prediccion = row_data.drop(columns=[TARGET_COL], errors='ignore').iloc[0].copy() # Convertir a Serie
    row_prediccion = row_prediccion.drop(labels=['NIT', 'RAZON_SOCIAL'], errors='ignore')
    row_prediccion["ANO_DE_CORTE"] = ano_prediccion # Seteamos el año de predicción
    ...
    # Conversión robusta a DataFrame de una fila (CRÍTICO para OHE)
    ...
    if ganancia_anterior == 0:
        ...
        if pred_real >= 0:
            ...
            if diferencia > 0:
                 # Ejemplo: -10 a -5 (Mejora, menos pérdida)
                delta_display = f"Pérdida REDUCIDA {abs(delta_percent_mag):,.2f}%"
            else:
                 # Ejemplo: -5 a -10 (Empeora, más pérdida)
