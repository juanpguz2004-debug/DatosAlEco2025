...
with tab3:
    ...
    st.markdown("""
        ...
        El Score de Riesgo (OE2), las Anomalías (OE1) y las métricas de Completitud 
    ...
    pregunta_usuario = st.text_input("Pregunta al Agente (e.g., ¿Cuál es la entidad que mejor cumple?):", "Muéstrame los 5 activos más usados que no cumplen con la frecuencia.")
    st.button("Consultar Agente")
    
    if pregunta_usuario:
        st.success(f"Analizando tu consulta: '{pregunta_usuario}'... (En el proyecto final, el LLM respondería aquí usando las métricas).")
